...
- [ ] **ToS/Privacy Policy acceptance capture** — nothing currently records that a user agreed to these documents or which version. Needs schema addition (e.g. `users.tos_accepted_at`, `users.tos_version`) plus a signup-flow checkbox.
- [ ] **Explicit consent for ID document submission** — POPIA treats this as special personal information requiring its own consent basis, separate from general ToS acceptance. Needs its own UI/consent capture, not just a blanket ToS checkbox.
...
- [ ] **Schedule the two maintenance jobs** — `app/jobs/purge_identity_documents.py` (POPIA retention) and `app/jobs/unpublish_expired_listings.py` (lapsed subscription cleanup) exist as scripts but are not wired to a scheduler (cron or APScheduler) anywhere yet.

### Should resolve soon after launch
- [ ] **Admin access tiering** — currently anyone with `role=admin` has full admin rights (verification review, message/report moderation, user suspension/ban). No distinction between e.g. a content moderator and someone who can ban accounts.
...
- [ ] **Refund/cancellation policy** — ToS Section 7.4 flags this as unresolved; South African consumer protection law (CPA) may impose specific requirements not yet reflected in the code or the document.
